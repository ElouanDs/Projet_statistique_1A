...
Pour commencer notre analyse, nous allons regarder le profil des élèves qui n'ont pas d'indices de REE globale ou de REE ciblée sur les professeurs de mathématiques. 


<<<<<<< HEAD
![Tableau 1: Comparaisons groupes avec valeurs manquantes et sans](images_rapports/tableau1_manquants_NA.png){fig-cap="Tableau 1 : Comparaison des groupes avec valeurs manquantes et sans"}

Note de lecture: Dans le groupe qui n'a pas de valeur en indice de REE globale ou de REE ciblée sur les professeurs de mathématiques on a 45.96% de filles. Pour le groupe avec aucune valeur manquante sur ces deux indices on a 51.18% de filles.
=======
| Groupe | % Filles | Statut socio-éco. | Anxiété en maths | Appartenance | Harcèlement | Effectif total |
|:-------|:--------:|:-----------------:|:----------------:|:------------:|:-----------:|:--------------:|
| Données manquantes | 45.96 | -0.67 | 0.31 | -0.21 | -0.12 | 159 590 |
| Sans NA | 51.18 | -0.20 | 0.22 | -0.05 | -0.31 | 454 154 |

: Comparaison des groupes selon la présence de données manquantes
>>>>>>> 0096db43b3324ef508b2f10baaccac56df8ff1be


Ce tableau sépare les données en deux : en premier nous avons les "manquant", c'est à dire les élèves n'ayant pas de REE globale ou de REE ciblée sur les professeurs de mathématiques, et les "sans NA".

<<<<<<< HEAD
Les "manquants" représentent 159590 données c'est-à-dire 26% de nos données. A partir de cela nous pouvons définir un premier profil des élèves qui ne se positionnent pas sur leur relation avec le corps enseignant. En premier lieu, le groupe "manquant" comprend plus de garçons que le groupe qui se positionne sur leur relation avec le corps enseignant, et est légèrement plus anxieux en mathématiques.  Trois indices ont une différence plus marquée. En effet, le premier montre que le sentiment d'appartenance pour le groupe "manquant" est plus faible que l'autre groupe avec un écart de 0.16 points. Le harcèlement augmente aussi, passant de -0.30 à -0.12 et enfin l'indice sur l'origine socio-économique diminue fortement, passant de -0.20 pour le groupe ayant répondu aux question, à -0.67 pour le groupe "manquant". 

Pour faire un résumé nos données manquant ont le profil suivant par rapport aux autres données: on y retrouve plus de garçons, plus anxieux envers les mathématiques, plus harcelés et avec un moins bon sentiment d'appartenance et pour finir une origine socio-économique beaucoup plus faible.
=======
...
<<<<<<< HEAD
Concernant la figure 3, nous avons la distribution des notes en mathématiques selon leur réponses ou non pour avoir un indice de REE centré sur les professeurs de mathématiques. En effet, l'utilisation de la moyenne en mathématiques est plus appropriée que la moyenne générale quand on regarde une potentielle liaison entre le professeur de mathématiques et l'élève. On en tire les mêmes conclusions que dans la figure 2. Les élèves n'ayant pas répondu aux questions relatives à la REE sur le professeur de mathématiques ont de moins bonnes notes que ceux ayant répondu aux questions dans cette matière.
=======
Concernant la figure 3, nous avons la distribution des notes en mathématiques selon leur réponses ou non pour avoir un indice de REE centré sur les professeurs de mathématiques. En effet l'utilisation de la moyenne en mathématiques est plus appropriée que la moyenne générale quand on regarde une potentielle liaison entre le professeur de mathématique et l'élève. On en tire les mêmes conclusion que dans la figure 2. Les élèves n’ayant pas répondu aux questions relatives à la REE avec le professeur de mathématiques ont de moins bonnes notes que ceux ayant répondu aux questions dans cette matière.
>>>>>>> 0096db43b3324ef508b2f10baaccac56df8ff1be

Nous pouvons donc ajouter des caractéristiques au profil de nos élèves qui ne se positionnent pas sur leur relation avec le corps enseignant. En effet, ce sont des élèves avec de moins bons résultats en général et en mathématiques. Ce profil montre que nos analyses futures entre la REE et les autres variables pourront être biaisées car on ne s'intéressera qu'aux élèves ayant un indice. On pourrait donc observer de moins fortes corrélations entre les notes et la REE car ceux ayant de moins bonnes notes sont ceux n'ayant pas de REE. 


| | REE | REE maths | Statut socio-éco. | Bien-être | Appart | Harcèlement | Anxiété maths |
|:--------|:---------------:|:-------------:|:-----------------:|:---------:|:------------:|:-----------:|:-------------:|
| *Min* | -10.09 | -3.254 | -6.841 | -3.127 | -3.504 | -1.228 | -2.626 |
| *1er Qu.* | -0.67 | -0.564 | -1.054 | -0.645 | -0.646 | -1.228 | -0.379 |
| *Médiane* | -0.011 | -0.1 | -0.184 | 0.126 | -0.313 | -0.391 | 0.193 |
| *Moyenne* | 0.04 | 0.118 | -0.31 | -0.001 | -0.081 | -0.268 | 0.232 |
| *3e Qu.* | 0.67 | 1.556 | 0.587 | 1.051 | 0.352 | 0.538 | 0.789 |
| *Max* | 3.417 | 1.649 | 7.38 | 1.117 | 3.173 | 4.694 | 2.892 |

<<<<<<< HEAD
Notes de lecture: La variable RELATST correspondant à la qualité de la REE globale possède une moyenne de -0.007, un minimum de -10.09 et un maximum de 3.417.

Notes: RELATST correspond à l'indice sur la REE globale.
TEACHSUP correspond à l'indice sur la REE ciblée en mathématique.
ESCS correspond à l'indice de l'origine socio-économique de l'élève.
EXPWB correspond à l'indice sur le bien-être de l'élève sur les derniers jours.
BELONG correspond à l'indice sur son sentiment d'appartenance.
BULLIED correspond à l'indice sur le harcèlement.
ANXMAT correspond à l'indice sur l'anxiété en mathématiques.
=======
: Statistiques descriptives des variables principales. *Bien-être* = bien-être des derniers jours. *Appart* = Sentiment d'appartenance

>>>>>>> 0096db43b3324ef508b2f10baaccac56df8ff1be

Pour continuer notre analyse, nous avons supprimé de notre base de données tous les élèves qui n'ont pas d'indice de REE globale et ciblée sur les mathématiques, puis avons vérifié que cela n'ait pas trop modifié les statistiques initiales de notre jeu de données. Selon les indications de l'OCDE, décrites dans la partie 1.1, la moyenne est centrée sur 0 pour les pays appartenant à l'OCDE et avec un écart-type de 1, donc en ajoutant les autres pays, on se retrouve proche de 0 mais pas forcément exactement. En enlevant les données, on retrouve partout une moyenne aux alentours de 0 puis avec un premier et troisième quartile entre -1 et 1 ce qui confirme que notre écart-type est toujours proche de 1. Ainsi, le fait d'avoir enlevé les données manquantes n'a pas trop affecté nos variables.
...
Il paraît maintenant pertinent de s'intéresser à la qualité de cette relation et savoir comment elle est perçue par les élèves ayant un indice REE. Car un indice relève en effet de la qualité globale de la relation, mais il est aussi important de savoir le détail par question. Pour cela nous avons récupéré le taux des réponses positives par question en ce qui concerne la relation élève-enseignant au global et celle avec les professeurs de mathématiques, seulement pour les élèves ayant répondu à la question. 
...
Les résultats sont très différents de ce qu'on a pu observer avec les enseignants dans leur globalité. En effet, les taux de réponses positives se trouvent seulement entre 25.4% et 32.2%, on se retrouve donc avec une majorité de réponses négatives contrairement à la relation globale. Cela montre donc que la REE avec le professeur de mathématiques est mauvaise comparativement à la REE globale.

Ainsi, après avoir caractérisé la relation élèves-enseignants et la manière dont elle est perçue par les élèves, il convient désormais de s’interroger quant à ses répercussions sur d'autres variables. C'est pourquoi, une des premières variables d'intérêt que nous pouvons étudier n'est autre que la réussite scolaire.
...
Comme la REE représente des interactions ayant lieues au sein des établissements scolaires, il est tout à fait naturel de s'interroger quant à l'influence de la REE sur le niveau des élèves et inversement.
...
Pour comprendre au mieux la relation que le niveau scolaire peut entretenir avec la REE, les analyses que nous mènerons, porterons sur la moyenne générale obtenue par les élèves ainsi que leur moyenne de mathématiques.
...
La relation élèves-enseignants générale est corrélée positivement à la relation élèves-enseignants en mathématiques (0.33).
Cette corrélation permet de voir que la REE en mathématiques s'explique en partie par la REE globale.
...
D’autres facteurs semblent donc être à la source de la réussite scolaire des élèves plutôt qu’une bonne REE globale seulement.

## 2.2 Le niveau académique national est corrélée négativement avec la qualité de la REE

Comme on constate qu'il n'y a pas de lien entre REE et les notes pour les élèves, il convient alors d'étudier cette relation à l'échelle des pays pour voir si des différences se font remarquer.
Dans cette analyse, on utilise la moyenne de REE dans chaque pays et la moyenne des notes obtenues.
<<<<<<< HEAD
Cela permet de comparer les niveaux nationaux et donc de l'influence des différents systèmes éducatifs sur la REE.
...
=======
Cela permet de comparer les niveaux nationaux et donc de comparer les différents systèmes éducatifs et leur influence sur la REE.

Il existe une corrélation négative moyenne entre la qualité de la REE et la note moyenne de chaque pays (r =\~ -0,26, p-value =\~ 0.03).
>>>>>>> 0096db43b3324ef508b2f10baaccac56df8ff1be
La REE n'explique que partiellement le niveau académique mais d'autres facteurs entrent en compte.
On ne peut donc pas conclure d'une relation très forte entre ces deux indicateurs.

En revanche, si l'on se concentre sur la REE avec les enseignants de mathématiques, on observe des résultats plus marqués.
La note moyenne utilisée est désormais celle en mathématiques et non la moyenne générale, même si les deux moyennes sont interchangeables.
...
<<<<<<< HEAD
Certains pays ne semblent pas suivre cette corrélation négative, comme Singapour où la relation avec les enseignants de mathématiques est plus positive que la moyenne des pays de l'OCDE d'environ 0.4 point.
=======
Certains pays ne semblent pas suivre cette corrélation négative, comme Singapour où la relation avec les enseignants de mathématiques est plus positive d'environ 0.4 point qu'en moyenne dans les pays de l'OCDE.
>>>>>>> 0096db43b3324ef508b2f10baaccac56df8ff1be
...
![Boxplots de la valeurs de l'indice de REE global selon l'indice socio_économique](images_rapports/figure9_boxplot_REE_ESCS.pdf){fig-cap="Boxplots de la valeur de l'indice de REE globale selon l'indice socio_économique"}
...
Le vécu des élèves apparaît comme similaire quelles que soient les origines sociales.
...
![Boxplots représentant la valeur de la REE ciblée sur l'enseignant de mathématiques en fonction de la valeur de l'indice socio-économique ](images_rapports/figure10_boxplot_REE_maths_ESCS.pdf){fig-cap="Boxplots représentant la valeur de la REE ciblée sur l'enseignant de mathématiques en fonction de la valeur de l'indice socio-économique"}
...
La "boîte" du premier quartile est plus haute.
...
Alors, lorsque dans un pays le statut socio-économique moyen a tendance à être plus défavorisé, les élèves observent en moyenne une meilleure relation avec leurs enseignants.
Inversement, dans les pays avec un statut socio-économique moyen plus important, la REE est jugée plus négativement selon les élèves.
=======
A l'échelle des pays, la REE et le statut socio-économique des élèves sont corrélés négativement de manière significative (r =\~ -0.41, p-value =\~ 0.0005).
...
![Nuage de points représentant les pays en fonction de leur REE moyen et de leur statut socio-économique](images_rapports/figure11_correlation_REE_statut.pdf){fig-cap="Nuage de points représentant les pays en fonction de leur REE moyenne et de leur statut socio-économique"}
...
En effet, si à l'échelle individuelle, on ne constate pas de lien de corrélation significative entre les origines sociales et la qualité de la REE, en comparant les différents pays, on peut conclure d'une influence.
...
<<<<<<< HEAD
D'un côté, on retouve des pays dont les élèves ont des origines moins favorisées et une REE plutôt positive, de l'autre côté un pays dont les élèves ont un statut scoio-économique correspondant à la moyenne OCDE et une REE jugée plus négativement qu'en moyenne au sein des pays de l'OCDE.
=======
D’un côté, on retrouve des pays dont les élèves sont issus de milieux plus défavorisés mais bénéficient d'une REE plutôt positive ; de l’autre, un pays dont les élèves ont un statut socio-économique proche de la moyenne de l'OCDE, mais où la REE est jugée plus négative que la moyenne.
>>>>>>> 0096db43b3324ef508b2f10baaccac56df8ff1be

On peut en conclure que si, au niveau individuel, aucune corrélation significative n'est observée, une corrélation modérée voire forte apparaît au niveau agrégé, c'est-à-dire au niveau des pays.
...
Comme le présentent de nombreuses études, le statut socio-économique des parents est positivement corrélé au niveau scolaire.
...
Tandis que la dimension 2 s'explique par la moyenne générale et le statut socio-économique, qui figurent comme étant corrélés positivement.
...
<<<<<<< HEAD
L'appréciation des mathématiques comme étant l'une des matières préférées de l'élève est légèrement corrélée à la qualité de la relation avec l'enseignement de cette matière.
...
Ainsi, aimer les maths ne suffit pas à expliquer une bonne relation entre un élève et son enseignant de mathématiques.
=======
L'appréciation des mathématiques comme étant l'une des matières préférées de l'élève est légèrement corrélée avec la qualité de la relation avec l'enseignement de cette matière.
La corrélation a un r d'environ 0.21 (p-value < 2.2e-16), ce qui permet de dire qu'il y a bel et bien un lien entre ces deux éléments mais celui-ci reste faible.
Alors, aimer les maths ne suffit pas à expliquer une bonne relation entre un élève et son enseignant de mathématiques.
>>>>>>> 0096db43b3324ef508b2f10baaccac56df8ff1be

La qualité perçue des cours de mathématiques est davantage corrélée à la qualité de la relation avec l'enseignant de mathématiques.
En effet, la corrélation est de 0.41 et significative (p-value < 2.2e-16).
...
<<<<<<< HEAD
Lorsqu'on change d'échelle, en regardant la qualité observée moyenne des cours de mathématiques par pays, on constate une corrélation positive marquée entre qualité des cours et la qualité de la REE (r² =\~ 0.62, p-value = 1.035e-08).
Ainsi, meilleure est la qualité des cours de mathématiques, plus la relation entre les élèves et les enseignants à l'échelle nationale est bonne.
=======
...
Note de lecture : Le Chili suit la relation de corrélation entre la qualité des cours de mathématiques et la qualité de la REE. Son indice de REE moyen est de 0.27 et son score moyen de qualité d'enseignement des mathématiques est de 7.

Avec ce nuage de points, on retrouve la même répartition des différents pays en miroir par rapport aux corrélations entre les notes et le statut socio-économique. On peut alors se demander s'il n'y aurait pas des profils types de pays selon la qualité de REE. Pour déterminer cela, nous avons réalisé un regroupement des pays selon leur indice de qualité de la REE, leur note moyenne et la qualité des cours de mathématiques. 
...
<<<<<<< HEAD
Tableau 2: Tableau des moyennes pour chaque groupe issu du regroupement k-means
=======
>>>>>>> 0096db43b3324ef508b2f10baaccac56df8ff1be
...
Il comprend le Viet Nam, le Pérou ou encore l'Arabie Saoudite.
...
<<<<<<< HEAD
Le groupe 3, en vert, avec des pays comme la Jordanie, la Thailande et le Brésil, dispose d'une qualité de REE neutre, des notes plus faibles qu'en moyenne (402) et une qualité d'enseignement en mathématiques légèrement supérieure à la moyenne (6.8).
=======
Le groupe 3, en vert, avec des pays comme la Jordanie, la Thailande et le Brésil, a une qualité de REE neutre, des notes plus faibles qu'en moyenne (402) et une qualité d'enseignement en mathématiques légèrement supérieure à la moyenne (6.8).
>>>>>>> 0096db43b3324ef508b2f10baaccac56df8ff1be
...
On peut alors se prononcer sur des profils types de pays. Un pays du groupe 1 semble avoir un système éducatif davatange axé sur la qualité de la REE et de l'enseignement, plutôt que sur la performance académique. Pour un pays du groupe 2, la qualité de la REE est généralement négative, pour autant le niveau académique reste légèrement supérieur à la moyenne. Concernant le groupe 3, les pays ont tendance à avoir une qualité de REE neutre, correspondant à la moyenne des pays de l'OCDE, ainsi qu'un niveau scolaire relativement faible. La qualité de l'enseignement est certes supérieure à la moyenne mais les pays de ce groupe ne se caractérisent pas par un profil très marqué. Enfin un pays du groupe 4 est avant tout un pays performant où la qualité de la REE est légèrement plus positive qu'en moyenne mais où la qualité de l'enseignement est également plus faible.

Pour mieux se rendre compte des regroupements de pays selon leur qualité de REE, notes et qualité d'enseignement, il est pertinent de les représenter sur une carte.
...
Si au niveau global, la REE ne semble pas avoir de lien avec la réussite scolaire, peut-être que celle-ci a un impact sur d'autres variables. Nous étudierons ainsi dans quelle mesure cette relation d'enseignant à élève peut impacter le bien-être de ces derniers.
...
Dans la suite de notre étude, nous définirons le bien-être de l'élève comme la moyenne de chacun des indices cités ci-dessus.

En outre, pour savoir si ces indices ont un réel impact sur les différents types de REE nous nous intéressons à la matrice des corrélations suivante :
...
Note : REE correspond à l'indice de qualité de ma relation élève-enseignant. REE maths est l'indice de qualité de la relation avec l'enseigant de mathématiques. Le bien-être fait référence à l'indice de bien-être récent et non au bien-être global. L'anxiété maths est l'anxiété ressentie pendant les cours de mathématiques. Appartenance correspond au sentiment d'appartenance des élèves et harcèlement est l'indice de harcèlement construit par l'OCDE.
...
Nous pouvons donc relever, que de petits liens existent seulement entre le bien-être et la REE, sous le prisme des élèves pris au cas par cas. Cependant, même si on ne peut tirer d’analyses très concluantes au niveau individuel, il peut être intéressant de changer de dimension et de se placer à l’échelle d’un pays tout entier. Cela pourrait nous permettre de déduire si, les méthodes d’interaction mises en place par les enseignants d’un pays aux yeux de leur élèves, ont un impact global sur le bien-être des élèves du pays concerné.
...
Comme défini dans la partie précédente, nous mesurons ici le bien-être par un indicateur général, construit à partir des quatres variables caractérisant le bien-être : le sentiment d’appartenance à l’école, le bien-être récent, l’anxiété en mathématiques et l’exposition au harcèlement. Les deux dernières dimensions étant négativement corrélées au bien-être, sont inversées afin d’assurer une interprétation homogène de l’indicateur. Ainsi, un score élevé de bien-être traduit systématiquement une situation plus favorable. En regroupant les individus simplement par pays, lorsque l’on calcule les liens de corrélation en faisant la moyenne de la REE des pays et de leur bien-être, on se retrouve dans la même situation que précédemment puisqu’aucun travail permettant de séparer l’échantillon a été effectué. En effet, on trouve un coefficient de corrélation pratiquement nul : -0.001. 
...
Un autre facteur permettant de séparer la population est représenté par le sexe. Ainsi l’objectif est d’identifier d’éventuelles inégalités ou sensibilités spécifiques des élèves selon leur sexe auprès de l'influence des REE sur leur bien-être.

Dans un premier temps, les observations avec une information manquante sur le sexe sont supprimées afin de garantir la comparabilité des groupes. Une variable catégorielle sexe est ensuite créée. Dans notre jeu de données nous avons 232437 filles (51%) contre 221685 garçons (49%), cela est donc équilibré.
...
Pour cela nous allons classer les pays en deux groupes selon que leur REE moyenne est meilleure pour les  garçons ou pour les filles. On calcule ensuite le bien-être moyen pour chacun de ces groupes.


<<<<<<< HEAD
Note de Lecture: Pour les pays où la REE est meilleures pour les garçons, le bien-être moyen est de 0.025
=======
| Groupe de pays | Bien-être moyen |
|:-------------------------------|:---------------:|
| REE favorable aux filles | 0.025 |
| REE favorable aux garçons | -0.014 |
>>>>>>> 0096db43b3324ef508b2f10baaccac56df8ff1be

: Bien-être moyen selon le profil de REE du pays


Le tableau laisse apparaître un écart entre les deux groupes, les pays à REE favorable aux filles affichant un bien-être moyen légèrement supérieur. Cet écart doit toutefois être soumis à un test statistique avant d'en tirer des conclusions.
...
| Groupe de pays | Bien-être moyen |
|:-------------------------------|:---------------:|
| REE meilleure chez les défavorisés | 0.029 |
| REE meilleure chez les favorisés | 0.005 |

: Bien-être moyen selon le profil de REE du pays distingué par catégories socio-économique
