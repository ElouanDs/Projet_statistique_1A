...
Note de lecture: Le Danemark (DNK), qui est un pays très développé, possède une moyenne de bien-être par élève à 0.10 et une moyenne de REE par élève de 0.30.
...
Dans un premier temps, les observations avec une information manquante sur le sexe sont supprimées afin de garantir la comparabilité des groupes. Une variable catégorielle sexe est ensuite créée. Dans notre jeu de données nous avons 232437 filles contre 221685 hommes, cela est donc équilibré.

Ainsi, au niveau des coefficients de corrélation, nous avons généralement relevé que ceux-ci sont plutôt similaires que ce soit pour les hommes ou bien pour les filles. L’impact de la REE sur le bien-être ne diffère pas réellement en fonction du sexe, prise comme seule variable discrimiante de la population. 
...
L’interprétation des coefficients de corrélation montre des valeurs très faibles : 0.087 chez les filles et -0.032 chez les hommes. Le lien entre relation enseignant-élève et bien-être apparaît légèrement plus marqué chez les filles, suggérant un effet un peu plus important de la REE sur leur bien-être. Toutefois, ces coefficients restent très faibles, ce qui indique qu’une analyse séparée par sexe ne suffit pas à mettre en évidence une relation entre ces deux variables.
Afin d’aller plus directement à l’analyse des inégalités, des variables d’écart entre hommes et filles sont construites pour chaque pays.
Ces écarts s’interprètent simplement : une valeur positive indique un avantage en faveur des hommes, tandis qu’une valeur négative traduit un avantage en faveur des filles. 
...
Note de Lecture: Pour les pays ayant une REE moins favorable aux garçons, donc un coefficient de corrélation sur les différence plus proche de 0, la différence de bien-être en les hommes et les filles est de 0.09.
...
![Nuage de points représentant les pays en fonction de leur différence de REE entre hommes-filles avec la différence de bien-être entre catégories socio-économiques](images_rapports/figure20_nuage_points_ESCS.pdf){fig-cap="Nuage de points représentant les pays en fonction de leur différence de REE entre hommes-filles avec la différence de bien-être entre catégories socio-économiques"}
