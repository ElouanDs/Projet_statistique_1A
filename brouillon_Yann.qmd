...
\begin{table}[ht]
    \centering
    \caption{Comparaison des groupes selon la présence de données manquantes}
    \label{tab:manquants}
    \renewcommand{\arraystretch}{1.3}
    \begin{tabular}{lS[table-format=2.2]S[table-format=-1.2]S[table-format=1.2]S[table-format=-1.2]S[table-format=-1.2]S[table-format=6.0]}
        \toprule
        \textbf{Groupe} &
        \textbf{\% Filles} &
        \textbf{Statut socio-éco.} &
        \textbf{Anxiété en maths} &
        \textbf{Appartenance} &
        \textbf{Harcèlement} &
        \textbf{Effectif total} \\
        \midrule
        Données manquantes & 45.96 & -0.67 & 0.31 & -0.21 & -0.12 & 159590 \\
        Sans NA            & 51.18 & -0.20 & 0.22 & -0.05 & -0.31 & 454154 \\
        \bottomrule
    \end{tabular}
\end{table}
```
...
vars <- data_etude %>%
  ...
  na.omit()
colnames(vars) <- c("Bien-être", "REE", "REE maths", "Anxiété maths", "Appartenance", "Harcèlement")
```
...
```{r,echo=FALSE}
cor.test(subset(data_oecd_pays, OECD == 1)$REE,
    subset(data_oecd_pays, OECD == 1)$bien_etre)

cor.test(subset(data_oecd_pays, OECD == 0)$REE,
...
```
```{r, echo = FALSE}
# on teste les hypothèses de la régression linéaire

lm_model <- lm(bien_etre ~ REE + OECD, data = data_oecd_pays)

plot(lm_model, which = 1)  # Residuals vs Fitted (linéarité)

# Test de Shapiro-Wilk (plus adapté que KS pour ça)
...
library(car)
vif(lm_model)  # VIF > 5 ou 10 = problème
...
data_etude$niveau_dev <- dplyr::case_when(
  ...
  data_etude$CNT %in% c(
    "THA","QAZ","MEX","ARG","URY","COL","BRA","MDA","QUR",
...
cor.test(subset(data_dev_pays, niveau_dev == "Intermédiaire")$REE,
         subset(data_dev_pays, niveau_dev == "Intermédiaire")$bien_etre, use = "complete.obs")


cor.test(subset(data_dev_pays, niveau_dev == "Faible")$REE,
         subset(data_dev_pays, niveau_dev == "Faible")$bien_etre, use = "complete.obs")


```
```{r}
# Ajout des noms complets à data_dev_pays
data_dev_pays <- data_dev_pays %>%
  left_join(correspondance_pays, by = c("CNT" = "iso_a3")) %>%
  mutate(nom_complet = ifelse(is.na(nom_complet), CNT, nom_complet))
```
...
infographie_data <- diff_sexe %>%
  mutate(groupe_pays = if_else(diff_REE > 0, 
                               "REE favorable aux garçons", 
                               "REE favorable aux filles")) %>%
  group_by(groupe_pays) %>%
  summarise(moyenne_diff_bien_etre = mean(c(bien_etre_homme,bien_etre_femme), na.rm = TRUE))

# 2. Création du graphique simple (style "Data-Visualisation")

pdf("images_rapports/figure18bis_graph_barre_sexe.pdf", width = 5, height = 3.3)
...
```{r}
bilan_pays_sexe <- bilan_pays_sexe %>%
  left_join(correspondance_pays, by = c("CNT" = "iso_a3")) %>%
  mutate(nom_complet = ifelse(is.na(nom_complet), CNT, nom_complet))
```
...
pdf("images_rapports/figure19_nuage_points_sexe.pdf", width = 7, height = 5)

ggplot(bilan_pays_sexe, aes(x = REE, y = bien_etre, color = sexe)) +
  geom_point(size = 2.5, alpha = 0.8) +
  geom_smooth(method = "lm", se = FALSE, linetype = "dashed", aes(group = sexe)) +
  geom_text_repel(aes(label = nom_complet), size = 3,
                box.padding = 0.5,
                point.padding = 0.3,
                segment.color = "grey50",
                show.legend = FALSE) +
  scale_color_manual(values = c("Femme" = "#d7191c", "Homme" = "#2c7bb6")) +
  labs(
    title = "REE et bien-être par pays selon le sexe",
    x = "Relation Enseignant-Élève (REE)",
    y = "Bien-être moyen",
    color = ""
  ) +
  theme_minimal() +
...
```{r}
diff_REE_escs <- bilan_pays_escs %>%
  filter(statut_socioeco_cat %in% c("Q1", "Q4")) %>%
  pivot_wider(names_from = statut_socioeco_cat, values_from = c(REE, bien_etre)) %>%
  mutate(
    diff_REE = REE_Q4 - REE_Q1,
    type_REE = if_else(diff_REE > 0,
                       "REE meilleure chez les favorisés",
                       "REE meilleure chez les défavorisés")
  ) %>%
  select(CNT, type_REE)
```
```{r}
barplot_data_escs <- bilan_pays_escs %>%
  left_join(diff_REE_escs, by = "CNT") %>%
  group_by(type_REE) %>%
  summarise(bien_etre_moyen = mean(bien_etre, na.rm = TRUE), .groups = "drop")

pdf("images_rapports/figure20_barplot_REE_escs.pdf", width = 5, height = 3.5)

ggplot(barplot_data_escs, aes(x = type_REE, y = bien_etre_moyen, fill = type_REE)) +
  geom_col(width = 0.5) +
  scale_fill_manual(values = c(
    "REE meilleure chez les défavorisés" = "#d7191c",
    "REE meilleure chez les favorisés" = "#2c7bb6"
  )) +
  labs(
    title = "Bien-être moyen selon le profil de REE du pays",
    x = "",
    y = "Bien-être moyen"
  ) +
  theme_minimal() +
...
```{r}
bilan_pays_escs_2groupes <- bilan_pays_escs %>%
  filter(statut_socioeco_cat %in% c("Q1", "Q4")) %>%
  mutate(groupe = if_else(statut_socioeco_cat == "Q1",
                          "25% les plus défavorisés",
                          "25% les plus favorisés"))
bilan_pays_escs_2groupes <- bilan_pays_escs_2groupes %>%
  left_join(correspondance_pays, by = c("CNT" = "iso_a3")) %>%
  mutate(nom_complet = ifelse(is.na(nom_complet), CNT, nom_complet))
```
...
pdf("images_rapports/figure21_nuage_REE_bienetre_escs.pdf", width = 7, height = 5)

ggplot(bilan_pays_escs_2groupes, aes(x = REE, y = bien_etre, color = groupe)) +
  geom_point(size = 2.5, alpha = 0.8) +
  geom_smooth(method = "lm", se = FALSE, linetype = "dashed", aes(group = groupe)) +
  geom_text_repel(aes(label = nom_complet), size = 3,
                box.padding = 0.6,
                point.padding = 0.3,
                segment.color = "grey50",
                show.legend = FALSE) +
  scale_color_manual(values = c(
    "25% les plus défavorisés" = "#d7191c",
    "25% les plus favorisés" = "#2c7bb6"
  )) +
  labs(
    title = "REE et bien-être par pays selon le milieu social",
    x = "Relation Enseignant-Élève (REE)",
    y = "Bien-être moyen",
    color = ""
  ) +
